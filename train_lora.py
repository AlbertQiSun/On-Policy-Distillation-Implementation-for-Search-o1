...
class Reasoner:
    ...
    def _load_model(self):
        ...
        if self.config.use_lora:
            ...
            logger.info("Applied LoRA to the student model.")
        ...
        self.optimizer = AdamW(filter(lambda p: p.requires_grad, self.student_model.parameters()), lr=5e-6)
    ...
    def generate_response(self, sequence: str) -> str:
        ...
        # Decode response
        generated_text = self.student_tokenizer.decode(outputs[0][initial_length:], skip_special_tokens=True)
        ...
        kl_coeff = 0.02  # 从config获取，初始0.01，根据监控调整（论文无此，但作为正则扩展）
    ...
    def save_model(self):
        save_path = f"{self.config.output_dir}/on_policy_distillation"
        if self.config.use_lora:
            save_path += "_lora"
        ...
        logger.info(f"Final model and plots saved to {save_path}")
    ...
    def _update_training_metrics(self, ppo_loss, surr1, surr2, advantages, ratios, kl_divergence, kl_loss):
        """Update training metrics for plotting."""
        epsilon = 0.2

        # Convert tensors to numpy for storage
        self.training_metrics['ppo_loss'].append(float(ppo_loss.item()) if torch.is_tensor(ppo_loss) else float(ppo_loss))
        self.training_metrics['surr1_loss'].append(float(surr1.mean().item()) if torch.is_tensor(surr1) else float(surr1))
        self.training_metrics['surr2_loss'].append(float(surr2.mean().item()) if torch.is_tensor(surr2) else float(surr2))

        # Store all advantage values
        adv_values = advantages.detach().cpu().numpy().flatten() if torch.is_tensor(advantages) else np.array(advantages).flatten()
        self.training_metrics['advantages'].extend(adv_values.tolist())

        # Store all ratio values
        ratio_values = ratios.detach().cpu().numpy().flatten() if torch.is_tensor(ratios) else np.array(ratios).flatten()
        self.training_metrics['ratios'].extend(ratio_values.tolist())

        # Store KL divergence values
        kl_values = kl_divergence.detach().cpu().numpy().flatten() if torch.is_tensor(kl_divergence) else np.array(kl_divergence).flatten()
        self.training_metrics['kl_divergence'].extend(kl_values.tolist())

        # Store KL loss value
        kl_loss_value = float(kl_loss.item()) if torch.is_tensor(kl_loss) else float(kl_loss)
        self.training_metrics['kl_loss'].append(kl_loss_value)

        # Calculate fraction of clipped ratios
        clipped_count = np.sum((ratio_values < 1 - epsilon) | (ratio_values > 1 + epsilon))
        total_count = len(ratio_values)
        clipped_fraction = clipped_count / total_count if total_count > 0 else 0
        self.training_metrics['clipped_ratio_fraction'].append(clipped_fraction)
